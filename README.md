...
### Homepage:
    ...
    * Top>> navbar
        ...
        - Enrolled/Registered Account:
            -- +2 extra pages: Post A Job, Profile

    * Hero banner >>
        - to the left, an image built with pixel transition component (hover effect: one side is the large logo, otherside is a black screen with some message/information about out platform)

        - to the right, 3 options:
            * login via Google, login using email/password, register (guest account)
            * 2 buttons: Post A job, Applied Jobs
    * Card >>
        - dynamically generated 6 cards displaying different job postings from various categories with some relevant information and job-related cover image (all loaded from the database via backend)

    * Scrolling card>>
        - Infinitely displaying the user reviews (cards with user names, short comments, account images)
    * Footer>>
        - Simple, static
